...
def _find_catalogue_matches(row: dict, top_k: int = 8, min_overall: int = 62,
    ...
    for cand in result["candidates"]:
        ...
        out.append({
            ...
            "material_view_b64": item.get("material_view_b64"),
            "has_material_view": bool(item.get("material_view_b64")),
...
def _studio_record_to_search_item(rec: dict) -> dict:
    ...
    return {
        ...
        # 2026-02-14 — photorealistic Nano Banana render (populated by
        # the material_view backfill; may be absent on newly published
        # records until the async task catches up). Frontend renders a
        # "Catalogue view / Material view" toggle only when this exists.
        "material_view_b64": rec.get("material_view_b64"),
...
# ---------------------------------------------------------------------------
# Material View backfill (2026-02-14). Photorealistic view of the physical
# material, generated by Gemini Nano Banana (`gemini-3.1-flash-image-preview`)
# using the actual flat swatch crop as the reference image. Runs alongside
# the visual_dna backfill: idempotent, single-run-at-a-time, safe to fire
# after any publish. Results are cached on `ke_records.material_view_b64`.
#
# Feasibility test 2026-02-14: Nano Banana preserved swatch fidelity where
# GPT-Image-1 hallucinated (marble → stucco). Only Nano Banana runs here.
# ---------------------------------------------------------------------------
_MATERIAL_VIEW_BACKFILL_LOCK: Optional["asyncio.Lock"] = None
_MATERIAL_VIEW_PROGRESS: dict = {
    "running": False,
    "pending": 0,
    "processed": 0,
    "generated": 0,
...
async def _material_view_backfill(limit: Optional[int] = None) -> None:
    """Generate Material Views for every published record with a swatch
    crop but no cached view. Concurrency capped at 2 to respect the
    Nano Banana proxy budget. Progress dict is exposed via
    `GET /api/admin/material-views/status`."""
    import asyncio
    from intelligence.material_view import generate_material_view
    global _MATERIAL_VIEW_BACKFILL_LOCK, _MATERIAL_VIEW_PROGRESS
    if _MATERIAL_VIEW_BACKFILL_LOCK is None:
        _MATERIAL_VIEW_BACKFILL_LOCK = asyncio.Lock()
    if _MATERIAL_VIEW_BACKFILL_LOCK.locked():
        logger.info("material_view backfill: already running, skipping")
        return
    async with _MATERIAL_VIEW_BACKFILL_LOCK:
        q = {
            "status": "published",
            "page_preview_b64": {"$exists": True, "$ne": None},
            "$or": [
                {"material_view_b64": {"$exists": False}},
                {"material_view_b64": None},
            ],
        }
        cursor = db.ke_records.find(q).limit(limit) if limit else db.ke_records.find(q)
        docs = await cursor.to_list(3000)
        _MATERIAL_VIEW_PROGRESS.update({
            "running": True, "pending": len(docs), "processed": 0,
        ...
        if not docs:
            ...
            return
        logger.info("material_view backfill: %d record(s) to generate", len(docs))
        sem = asyncio.Semaphore(2)

        async def one(d: dict):
            async with sem:
                swatch = d.get("page_preview_b64") or ""
                png = await generate_material_view(d, swatch)
                _MATERIAL_VIEW_PROGRESS["processed"] += 1
                if not png:
                    _MATERIAL_VIEW_PROGRESS["failed"] += 1
                    return
                b64 = base64.b64encode(png).decode("ascii")
                await db.ke_records.update_one(
                    {"id": d["id"]},
                    {"$set": {
                        "material_view_b64": b64,
                        "material_view_generated_at": datetime.now(timezone.utc).isoformat(),
                        "material_view_model": "gemini-3.1-flash-image-preview",
                    }},
                )
                _MATERIAL_VIEW_PROGRESS["generated"] += 1

        results = await asyncio.gather(*(one(d) for d in docs), return_exceptions=True)
        errors = [r for r in results if isinstance(r, Exception)]
        if errors:
            _MATERIAL_VIEW_PROGRESS["last_error"] = str(errors[0])[:200]
            logger.warning("material_view backfill: %d exception(s), first: %s",
                           len(errors), errors[0])
        _MATERIAL_VIEW_PROGRESS.update({
            "running": False,
            "finished_at": datetime.now(timezone.utc).isoformat(),
        })
        logger.info("material_view backfill: done — generated %d, failed %d",
                    _MATERIAL_VIEW_PROGRESS["generated"],
                    _MATERIAL_VIEW_PROGRESS["failed"])
        await _refresh_studio_index()
...
# ---------------------------------------------------------------------------
# Material View admin endpoints (2026-02-14).
# ---------------------------------------------------------------------------
@api_router.get("/admin/material-views/status")
async def admin_material_view_status(admin: dict = Depends(require_admin)):
    """Progress snapshot of the current / most recent Material View
    backfill run. Also returns the DB-wide coverage numbers so the
    founder can eyeball how many records already have a cached view."""
    total_pub = await db.ke_records.count_documents(
        {"status": "published", "page_preview_b64": {"$exists": True, "$ne": None}})
    with_view = await db.ke_records.count_documents(
        {"status": "published", "material_view_b64": {"$exists": True, "$ne": None}})
    return {
        "coverage": {
            "published_with_swatch": total_pub,
            "with_material_view": with_view,
            "missing": max(total_pub - with_view, 0),
        },
        "progress": _MATERIAL_VIEW_PROGRESS,
    }


@api_router.post("/admin/material-views/backfill")
async def admin_material_view_backfill(
    limit: Optional[int] = None,
    admin: dict = Depends(require_admin),
):
    """Fire-and-return the Material View backfill. Non-blocking — poll
    `/admin/material-views/status` for progress. If a run is already in
    flight, this is a no-op (the endpoint is idempotent by design)."""
    import asyncio
    if _MATERIAL_VIEW_BACKFILL_LOCK and _MATERIAL_VIEW_BACKFILL_LOCK.locked():
        return {"queued": False, "reason": "already running",
                "progress": _MATERIAL_VIEW_PROGRESS}
    asyncio.create_task(_material_view_backfill(limit=limit))
    return {"queued": True, "limit": limit,
            "hint": "poll /admin/material-views/status for progress"}
...
async def startup_event():
    ...
    _aio.create_task(_material_view_backfill())
...
async def studio_bulk_records(
    ...
    if payload.action == "publish":
        ...
        asyncio.create_task(_material_view_backfill())
